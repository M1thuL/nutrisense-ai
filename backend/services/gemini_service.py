...
class GeminiService:
    def __init__(self):
        api_key = os.getenv("GEMINI_API_KEY", "dummy_key_for_build")
    ...
    def analyze_meal(self, meal_description: str) -> dict:
        ...
        try:
            ...
            # Clean up the response to extract JSON (in case it includes markdown block wrapper)
            if "```json" in text:
    ...
    def get_recommendations(self, time_of_day: str, recent_meals: list) -> list:
    ...
    def get_nutrition_facts(self, food_item: str) -> dict:
        prompt = f"""
